...
const fetchPage = async (slug: string) => {
	...
	const data = await fetch(`https://api.storyblok.com/v2/cdn/stories/${slug}?version=${process.env.NODE_ENV === "development" ? "draft" : "published"}&token=${process.env.STORYBLOCK_PREVIEW_ACCESS_TOKEN}`)
	const response = await data.json();
